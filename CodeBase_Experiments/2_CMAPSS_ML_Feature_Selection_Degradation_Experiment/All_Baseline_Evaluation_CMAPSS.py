...
def load_all_results():
    ...
    # Standardize CMAPSS column name
...
def plot_cmapss_evolution(df, model_family, output_dir, engine_range, range_label):
    print(f"\nPlotting CMAPSS evolution for {model_family} models ({range_label})...")
    ...
    line_endpoints = []  # Store (cmapss_at_10pct, model_name, color) for labeling
    ...
    for model in sorted(models):
        ...
        plt.plot(model_data['n_engines'], model_data['cmapss'],
                marker='o', linewidth=2.5, markersize=8,
                label=model, color=colors[model], alpha=0.9)
        
        # Get CMAPSS at lowest engine count in this range for labeling
        min_engines = min(engine_range)
        cmapss_at_min = model_data[model_data['n_engines'] == min_engines]['cmapss'].values
        if len(cmapss_at_min) > 0:
            line_endpoints.append((cmapss_at_min[0], model, colors[model]))
    
    # Add biology-style labels aligned vertically on the right with connector lines
    # Sort by CMAPSS to keep similar values closer
    ...
    if range_label == "high_engines":
        label_y_positions = np.linspace(7000, 34000, n_labels)
        ...
        label_y_positions = np.linspace(10000, 190000, n_labels)  # More space for low engine range
    ...
    for idx, (cmapss_val, model, color) in enumerate(line_endpoints):
        label_y = label_y_positions[idx]
        
        # Draw connector line from actual data point to label
        plt.plot([line_start_x, label_x], [cmapss_val, label_y], 
    ...
    plt.ylabel('CMAPSS Score (Validation Set)', fontsize=13, fontweight='bold')
    
    range_title = "High Engine Counts (80-10)" if range_label == "high_engines" else "Low Engine Counts (10-1)"
    plt.title(f'{model_family} Models: CMAPSS Score Evolution - {range_title}',
    ...
    if range_label == "high_engines":
        ...
        plt.ylim(5000, 200000)  # Much higher for low data (worse scores)
        plt.xlim(max(engine_range) + 1, min_engines - 3)  # Extra space on left for labels
    
    plt.tight_layout()
    
    # Save
    filename = f"{model_family.lower().replace('-', '_').replace(' ', '_')}_cmapss_evolution_{range_label}.png"
    output_file = output_dir / filename
    plt.savefig(output_file, dpi=300)
    print(f"Saved: {output_file}")
    plt.close()

def plot_combined_baseline_evaluation_cmapss(regression_df, tree_df, dl_df, output_dir, engine_range, range_label):
    print(f"\nPlotting combined baseline CMAPSS evaluation (All 3 families) ({range_label})...")
    ...
    min_engines = min(engine_range)
    if range_label == "high_engines":
        label_y_positions = np.linspace(6000, 34000, n_labels)
        label_x = min_engines - 2  # X position for labels (right side, beyond lowest data point)
        line_start_x = min_engines  # Start connector from lowest engine count in range
    else:  # low_engines
        label_y_positions = np.linspace(10000, 190000, n_labels)  # More space for low engine range
    ...
    plt.ylabel('CMAPSS Score (Validation Set)', fontsize=14, fontweight='bold')
    
    range_title = "High Engine Counts (80-10)" if range_label == "high_engines" else "Low Engine Counts (10-1)"
    plt.title(f'All Baseline Models: CMAPSS Score Evolution - {range_title}\n(Regression, Tree-Based, and Deep Learning)',
    ...
    if range_label == "high_engines":
        plt.ylim(5000, 35000)
        plt.xlim(max(engine_range) + 5, 0)
    else:  # low_engines
        plt.ylim(5000, 200000)
        plt.xlim(max(engine_range) + 1, 0)
    
    plt.tight_layout()
    
    # Save
    output_file = output_dir / f'combined_baseline_evaluation_cmapss_{range_label}.png'
    plt.savefig(output_file, dpi=300)
    print(f"Saved: {output_file}")
    plt.close()
...
def main():
    print("="*40)
    print("ALL BASELINE MODELS EVALUATION - CMAPSS Score")
    print("="*40)
    ...
    # Plot CMAPSS evolution for Regression models (high engines)
    plot_cmapss_evolution(regression_df, 'Regression', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot CMAPSS evolution for Tree-Based models (high engines)
    plot_cmapss_evolution(tree_df, 'Tree-Based', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot CMAPSS evolution for Deep Learning models (high engines)
    plot_cmapss_evolution(dl_df, 'Deep Learning', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot combined evaluation (high engines)
    plot_combined_baseline_evaluation_cmapss(regression_df, tree_df, dl_df, OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    print("\n" + "="*40)
    print("GENERATING LOW ENGINE COUNT PLOTS (10-1 engines)")
    print("="*40)
    
    # Plot CMAPSS evolution for Regression models (low engines)
    plot_cmapss_evolution(regression_df, 'Regression', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot CMAPSS evolution for Tree-Based models (low engines)
    plot_cmapss_evolution(tree_df, 'Tree-Based', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot CMAPSS evolution for Deep Learning models (low engines)
    plot_cmapss_evolution(dl_df, 'Deep Learning', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot combined evaluation (low engines)
    plot_combined_baseline_evaluation_cmapss(regression_df, tree_df, dl_df, OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    ...
    print("\nGenerated files:")
    print("1. regression_cmapss_evolution.png")
    print("2. tree_based_cmapss_evolution.png")
    print("3. deep_learning_cmapss_evolution.png")
    print("4. combined_baseline_evaluation_cmapss.png (ALL MODELS)")
    print("="*40)
