...
def plot_mae_evolution(df, model_family, output_dir, engine_range, range_label):
    print(f"\nPlotting MAE evolution for {model_family} models ({range_label})...")
    ...
    line_endpoints = []  # Store (mae_at_10pct, model_name, color) for labeling
    ...
    for model in sorted(models):
        ...
        plt.plot(model_data['n_engines'], model_data['mae'],
                marker='o', linewidth=2.5, markersize=8,
                label=model, color=colors[model], alpha=0.9)
        
        # Get MAE at 10% data for labeling
        mae_at_1 = model_data[model_data['n_engines'] == 1]['mae'].values
        if len(mae_at_1) > 0:
            line_endpoints.append((mae_at_1[0], model, colors[model]))
    
    # Add biology-style labels aligned vertically on the right with connector lines
    # Sort by MAE to keep similar values closer
    ...
    if range_label == "high_engines":
        label_y_positions = np.linspace(8, 19, n_labels)
        label_x = 3  # X position for labels (right side)
        line_start_x = 10  # Start connector from 10 engines
    else:  # low_engines
        label_y_positions = np.linspace(8, 48, n_labels)
        label_x = -1.5  # X position for labels (left side for low engines)
        line_start_x = 1  # Start connector from 1 engine
    
    for idx, (mae_val, model, color) in enumerate(line_endpoints):
        label_y = label_y_positions[idx]
        
        # Draw connector line from actual data point to label
        plt.plot([line_start_x, label_x], [mae_val, label_y], 
    ...
    plt.ylabel('MAE (Validation Set)', fontsize=13, fontweight='bold')
    
    range_title = "High Engine Counts (80-10)" if range_label == "high_engines" else "Low Engine Counts (10-1)"
    plt.title(f'{model_family} Models: MAE Evolution - {range_title}',
    ...
    if range_label == "high_engines":
        plt.ylim(7, 20)
        plt.xlim(max(engine_range) + 5, -2)  # Extra space on right for labels
    else:  # low_engines
        plt.ylim(7, 50)
        plt.xlim(max(engine_range) + 1, -3)  # Extra space on left for labels
    
    plt.tight_layout()
    
    # Save
    filename = f"{model_family.lower().replace('-', '_').replace(' ', '_')}_mae_evolution_{range_label}.png"
    output_file = output_dir / filename
    plt.savefig(output_file, dpi=300)
    print(f"Saved: {output_file}")
    plt.close()

def plot_combined_baseline_evaluation_mae(regression_df, tree_df, dl_df, output_dir, engine_range, range_label):
    print(f"\nPlotting combined baseline MAE evaluation (All 3 families) ({range_label})...")
    ...
    n_labels = len(line_endpoints)
    if range_label == "high_engines":
        label_y_positions = np.linspace(7.5, 19.5, n_labels)
        label_x = 2.5  # X position for labels (right side)
        line_start_x = 10  # Start connector from 10 engines
    else:  # low_engines
        label_y_positions = np.linspace(8, 48, n_labels)
        label_x = -1.5  # X position for labels (left side for low engines)
    ...
    plt.ylabel('MAE (Validation Set)', fontsize=14, fontweight='bold')
    
    range_title = "High Engine Counts (80-10)" if range_label == "high_engines" else "Low Engine Counts (10-1)"
    plt.title(f'All Baseline Models: MAE Evolution - {range_title}\n(Regression, Tree-Based, and Deep Learning)',
    ...
    # Save
    output_file = output_dir / f'combined_baseline_evaluation_mae_{range_label}.png'
    plt.savefig(output_file, dpi=300)
    print(f"Saved: {output_file}")
    plt.close()
...
def main():
    print("="*40)
    print("ALL BASELINE MODELS EVALUATION - MAE")
    print("="*40)
    ...
    # Plot MAE evolution for Regression models (high engines)
    plot_mae_evolution(regression_df, 'Regression', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot MAE evolution for Tree-Based models (high engines)
    plot_mae_evolution(tree_df, 'Tree-Based', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot MAE evolution for Deep Learning models (high engines)
    plot_mae_evolution(dl_df, 'Deep Learning', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot combined evaluation (high engines)
    plot_combined_baseline_evaluation_mae(regression_df, tree_df, dl_df, OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    print("\n" + "="*40)
    print("GENERATING LOW ENGINE COUNT PLOTS (10-1 engines)")
    print("="*40)
    
    # Plot MAE evolution for Regression models (low engines)
    plot_mae_evolution(regression_df, 'Regression', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot MAE evolution for Tree-Based models (low engines)
    plot_mae_evolution(tree_df, 'Tree-Based', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot MAE evolution for Deep Learning models (low engines)
    plot_mae_evolution(dl_df, 'Deep Learning', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot combined evaluation (low engines)
    plot_combined_baseline_evaluation_mae(regression_df, tree_df, dl_df, OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    ...
    print("\nGenerated files:")
    print("1. regression_mae_evolution.png")
    print("2. tree_based_mae_evolution.png")
    print("3. deep_learning_mae_evolution.png")
    print("4. combined_baseline_evaluation_mae.png (ALL MODELS)")
    print("="*40)
