...
OUTPUT_DIR = PROJECT_ROOT / 'Results' / 'Phase3_Feature_Extraction' / DATASET / 'TCN_LSTM'
...
def build_tcn_lstm_model(input_shape, tcn_config, lstm_config):
    ...
    model = Model(inputs=inputs, outputs=outputs, name='TCN_LSTM')
...
def optuna_objective(trial, train_data, val_data, lstm_config, selected_features):
    ...
    try:
        # Build model
        model = build_tcn_lstm_model(input_shape, tcn_config, lstm_config)
...
def optimize_tcn_hps(train_data, val_data, lstm_config, selected_features):
    print("\n" + "="*40)
    ...
    study = optuna.create_study(
        ...
        study_name='TCN_LSTM_HP_Optimization'
    )
...
def evaluate_all_percentages(best_tcn_config, lstm_config, selected_features, engine_counts):
    print("\n" + "="*40)
    ...
    for n_engines in engine_counts:
        ...
        try:
            ...
            # Build model
            model = build_tcn_lstm_model(input_shape, best_tcn_config, lstm_config)
            ...
            checkpoint_file = OUTPUT_DIR / 'tcn_lstm_all_results_checkpoint.csv'
            ...
            model.save(model_dir / f'tcn_lstm_{n_engines}n_engines.keras')
    ...
    results_file = OUTPUT_DIR / 'tcn_lstm_all_results.csv'
...
def main(test_mode=False):
    print("\n" + "="*40)
    print("TCN-LSTM FEATURE EXTRACTION EXPERIMENT")
    if test_mode:
    ...
    # Check if we already have best config
    best_config_file = OUTPUT_DIR / "best_tcn_config.json"
    ...
    final_results = evaluate_all_percentages(best_tcn_config, LSTM_FIXED_HPS, selected_features, engine_counts_to_use)
    
    # Save experiment metadata (for thesis documentation)
    metadata = {
        'experiment_name': 'TCN_LSTM_Feature_Extraction',
        'dataset': DATASET,
    ...
    # Final summary
    print("\n" + "="*40)
    print("TCN-LSTM EXPERIMENT COMPLETE!")
    print("="*40)
    ...
    print(f"tcn_lstm_all_results.csv - Full results across all data %")
