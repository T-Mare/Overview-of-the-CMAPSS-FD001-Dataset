...
OUTPUT_DIR = PROJECT_ROOT / 'Results' / 'Phase3_Feature_Extraction' / DATASET / 'AE_LSTM'
...
DROPOUT = AE_LSTM_FIXED_PARAMS['dropout']
...
def build_ae_lstm_model(input_shape, ae_config, lstm_config):
    timesteps, n_features = input_shape
    
    inputs = keras.Input(shape=input_shape, name='input')
    
    # Apply encoder to each timestep using TimeDistributed
    # This compresses features: (timesteps, n_features)  (timesteps, bottleneck_dim)
    x = inputs
    ...
    for i in range(ae_config['encoder_layers']):
        ...
        if DROPOUT > 0:
            x = layers.TimeDistributed(
                layers.Dropout(DROPOUT, name=f'encoder_dropout_{i+1}'),
                name=f'td_dropout_{i+1}'
            )(x)
    ...
    print(f"Encoder: {n_features} features  {ae_config['bottleneck_dim']} compressed features")
    
    # LSTM layers (fixed from Phase 1) - process compressed temporal sequence
    for i in range(lstm_config['num_layers']):
        ...
        if lstm_config['dropout'] > 0:
            x = layers.Dropout(lstm_config['dropout'], name=f'lstm_dropout_{i+1}')(x)
...
def optuna_objective(trial, train_data, val_data, lstm_config, selected_features):
    # Suggest AE hyperparameters from config.py search space
    ae_config = {}
    ...
    try:
        # Build model
        model = build_ae_lstm_model(input_shape, ae_config, lstm_config)
...
def optimize_ae_hps(train_data, val_data, lstm_config, selected_features):
    print("\n" + "="*40)
    print("HYPERPARAMETER OPTIMIZATION WITH OPTUNA (100% Data)")
    print("="*40)
    ...
    study = optuna.create_study(
        ...
        study_name='AE_LSTM_HP_Optimization'
    )
...
def evaluate_all_percentages(best_ae_config, lstm_config, selected_features, engine_counts):
    print("\n" + "="*40)
    ...
    for n_engines in engine_counts:
        print(f"\n{'='*40}")
        print(f"DATA PERCENTAGE: {n_engines}%")
        print(f"{'='*40}")
        ...
        try:
            ...
            # Build model
            model = build_ae_lstm_model(input_shape, best_ae_config, lstm_config)
            ...
            history_df.to_csv(history_dir / f'history_{n_engines}n_engines.csv', index=False)
            ...
            checkpoint_file = OUTPUT_DIR / 'ae_lstm_all_results_checkpoint.csv'
            ...
            model.save(model_dir / f'ae_lstm_{n_engines}n_engines.keras')
            ...
            print(f"Error at {n_engines}%: {e}")
            continue
    
    # Save all results
    results_df = pd.DataFrame(all_results)
    results_file = OUTPUT_DIR / 'ae_lstm_all_results.csv'
...
def main(test_mode=False):
    print("\n" + "="*40)
    print("AE-LSTM FEATURE EXTRACTION EXPERIMENT")
    if test_mode:
    ...
    final_results = evaluate_all_percentages(best_ae_config, LSTM_FIXED_HPS, selected_features, engine_counts_to_use)
    
    # Save experiment metadata (for thesis documentation)
    metadata = {
        'experiment_name': 'AE_LSTM_Feature_Extraction',
        'dataset': DATASET,
        ...
        'best_val_rmse_100n_engines': study.best_value,
        'total_optuna_trials_completed': len(study.trials),
    ...
    # Final summary
    print("\n" + "="*40)
    print("AE-LSTM EXPERIMENT COMPLETE!")
    print("="*40)
    ...
    print(f"best_ae_config.json - Best AE configuration found")
    print(f"ae_lstm_all_results.csv - Full results across all data %")
