...
OUTPUT_DIR = PROJECT_ROOT / 'Results' / 'Phase3_Feature_Extraction' / DATASET / 'CNN_GRU'
...
def build_cnn_gru_model(input_shape, cnn_config, gru_config):
    ...
    for i in range(gru_config['num_layers']):
        ...
        if gru_config['dropout'] > 0:
            x = layers.Dropout(gru_config['dropout'], name=f'dropout_{i+1}')(x)
...
def optuna_objective(trial, train_data, val_data, gru_config, selected_features):
    ...
    try:
        # Build model
        model = build_cnn_gru_model(input_shape, cnn_config, gru_config)
...
def optimize_cnn_hps(train_data, val_data, gru_config, selected_features):
    print("\n" + "="*40)
    ...
    study = optuna.create_study(
        ...
        study_name='CNN_GRU_HP_Optimization'
    )
...
def evaluate_all_percentages(best_cnn_config, gru_config, selected_features, engine_counts):
    print("\n" + "="*40)
    ...
    for n_engines in engine_counts:
        ...
        try:
            ...
            # Build model
            model = build_cnn_gru_model(input_shape, best_cnn_config, gru_config)
            ...
            checkpoint_file = OUTPUT_DIR / 'cnn_gru_all_results_checkpoint.csv'
            ...
            model.save(model_dir / f'cnn_gru_{n_engines}engines.keras')
    ...
    results_file = OUTPUT_DIR / 'cnn_gru_all_results.csv'
...
def main(test_mode=False):
    print("\n" + "="*40)
    print("CNN-GRU FEATURE EXTRACTION EXPERIMENT")
    if test_mode:
    ...
    final_results = evaluate_all_percentages(best_cnn_config, GRU_FIXED_HPS, selected_features, engine_counts_to_use)
    
    # Save experiment metadata (for thesis documentation)
    metadata = {
        'experiment_name': 'CNN_GRU_Feature_Extraction',
        'dataset': DATASET,
    ...
    # Final summary
    print("\n" + "="*40)
    print("CNN-GRU EXPERIMENT COMPLETE!")
    print("="*40)
    ...
    print(f"cnn_gru_all_results.csv - Full results across all engine counts")
