...
OUTPUT_DIR = PROJECT_ROOT / 'Results' / 'Phase3_Feature_Extraction' / DATASET / 'AE_GRU'
...
def build_ae_gru_model(input_shape, ae_config, gru_config):
    ...
    # GRU layers (fixed from Phase 1) - process compressed temporal sequence
    ...
    model = Model(inputs=inputs, outputs=outputs, name='AE_GRU')
...
def optuna_objective(trial, train_data, val_data, gru_config, selected_features):
    ...
    try:
        # Build model
        model = build_ae_gru_model(input_shape, ae_config, gru_config)
...
def optimize_ae_hps(train_data, val_data, gru_config, selected_features):
    ...
    study = optuna.create_study(
        ...
        study_name='AE_GRU_HP_Optimization'
    )
...
def evaluate_all_percentages(best_ae_config, gru_config, selected_features, engine_counts):
    print("\n" + "="*40)
    ...
    for n_engines in engine_counts:
        ...
        try:
            ...
            # Build model
            model = build_ae_gru_model(input_shape, best_ae_config, gru_config)
            ...
            checkpoint_file = OUTPUT_DIR / 'ae_gru_all_results_checkpoint.csv'
            ...
            model.save(model_dir / f'ae_gru_{n_engines}engines.keras')
    ...
    results_file = OUTPUT_DIR / 'ae_gru_all_results.csv'
...
def main(test_mode=False):
    print("\n" + "="*40)
    print("AE-GRU FEATURE EXTRACTION EXPERIMENT")
    if test_mode:
    ...
    final_results = evaluate_all_percentages(best_ae_config, GRU_FIXED_HPS, selected_features, engine_counts_to_use)
    
    # Save experiment metadata (for thesis documentation)
    metadata = {
        'experiment_name': 'AE_GRU_Feature_Extraction',
        'dataset': DATASET,
        ...
        'best_val_rmse_80engines': study.best_value,
    ...
    # Final summary
    print("\n" + "="*40)
    print("AE-GRU EXPERIMENT COMPLETE!")
    print("="*40)
    ...
    print(f"ae_gru_all_results.csv - Full results across all engine counts")
