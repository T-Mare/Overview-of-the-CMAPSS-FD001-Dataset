...
def plot_r2_evolution(df, model_family, output_dir, engine_range, range_label):
    print(f"\nPlotting R² evolution for {model_family} models ({range_label})...")
    ...
    line_endpoints = []  # Store (r2_at_10pct, model_name, color) for labeling
    ...
    for model in sorted(models):
        ...
        plt.plot(model_data['n_engines'], model_data['r2'],
                marker='o', linewidth=2.5, markersize=8,
                label=model, color=colors[model], alpha=0.9)
        
        # Get R² at lowest engine count in this range for labeling
        min_engines = min(engine_range)
        r2_at_min = model_data[model_data['n_engines'] == min_engines]['r2'].values
        if len(r2_at_min) > 0:
            line_endpoints.append((r2_at_min[0], model, colors[model]))
    
    # Add biology-style labels aligned vertically on the right with connector lines
    # Sort by R² (REVERSE - higher is better)
    line_endpoints.sort(key=lambda x: x[0], reverse=True)
    
    # Calculate evenly spaced positions for labels
    n_labels = len(line_endpoints)
    min_engines = min(engine_range)
    if range_label == "high_engines":
        label_y_positions = np.linspace(0.95, 0.65, n_labels)
        ...
        label_y_positions = np.linspace(0.95, -5.5, n_labels)  # More space for low engine range
    ...
    for idx, (r2_val, model, color) in enumerate(line_endpoints):
        label_y = label_y_positions[idx]
        
        # Draw connector line from actual data point to label
        plt.plot([line_start_x, label_x], [r2_val, label_y], 
    ...
    plt.ylabel('R² (Validation Set) - Higher is Better', fontsize=13, fontweight='bold')
    
    range_title = "High Engine Counts (80-10)" if range_label == "high_engines" else "Low Engine Counts (10-1)"
    plt.title(f'{model_family} Models: R² Evolution - {range_title}',
    ...
    if range_label == "high_engines":
        ...
        plt.ylim(-6.0, 1.0)  # Negative R² possible for very low data
        plt.xlim(max(engine_range) + 1, min_engines - 3)  # Extra space on left for labels
    
    plt.tight_layout()
    
    # Save
    filename = f"{model_family.lower().replace('-', '_').replace(' ', '_')}_r2_evolution_{range_label}.png"
    output_file = output_dir / filename
    plt.savefig(output_file, dpi=300)
    print(f"Saved: {output_file}")
    plt.close()

def plot_combined_baseline_evaluation_r2(regression_df, tree_df, dl_df, output_dir, engine_range, range_label):
    print(f"\nPlotting combined baseline R² evaluation (All 3 families) ({range_label})...")
    ...
    min_engines = min(engine_range)
    if range_label == "high_engines":
        label_y_positions = np.linspace(0.98, 0.62, n_labels)
        label_x = min_engines - 2  # X position for labels (right side, beyond lowest data point)
        line_start_x = min_engines  # Start connector from lowest engine count in range
    else:  # low_engines
        label_y_positions = np.linspace(0.95, -5.5, n_labels)  # More space for low engine range
    ...
    plt.ylabel('R² (Validation Set) - Higher is Better', fontsize=14, fontweight='bold')
    
    range_title = "High Engine Counts (80-10)" if range_label == "high_engines" else "Low Engine Counts (10-1)"
    plt.title(f'All Baseline Models: R² Evolution - {range_title}\n(Regression, Tree-Based, and Deep Learning)',
    ...
    # Save
    output_file = output_dir / f'combined_baseline_evaluation_r2_{range_label}.png'
    plt.savefig(output_file, dpi=300)
    print(f"Saved: {output_file}")
    plt.close()
...
def main():
    print("="*40)
    print("ALL BASELINE MODELS EVALUATION - R²")
    print("="*40)
    ...
    # Plot R² evolution for Regression models (high engines)
    plot_r2_evolution(regression_df, 'Regression', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot R² evolution for Tree-Based models (high engines)
    plot_r2_evolution(tree_df, 'Tree-Based', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot R² evolution for Deep Learning models (high engines)
    plot_r2_evolution(dl_df, 'Deep Learning', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot combined evaluation (high engines)
    plot_combined_baseline_evaluation_r2(regression_df, tree_df, dl_df, OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    print("\n" + "="*40)
    print("GENERATING LOW ENGINE COUNT PLOTS (10-1 engines)")
    print("="*40)
    
    # Plot R² evolution for Regression models (low engines)
    plot_r2_evolution(regression_df, 'Regression', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot R² evolution for Tree-Based models (low engines)
    plot_r2_evolution(tree_df, 'Tree-Based', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot R² evolution for Deep Learning models (low engines)
    plot_r2_evolution(dl_df, 'Deep Learning', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot combined evaluation (low engines)
    plot_combined_baseline_evaluation_r2(regression_df, tree_df, dl_df, OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    ...
    print("\nGenerated files:")
    print("1. regression_r2_evolution.png")
    print("2. tree_based_r2_evolution.png")
    print("3. deep_learning_r2_evolution.png")
    print("4. combined_baseline_evaluation_r2.png (ALL MODELS)")
    print("="*40)
