...
OUTPUT_DIR = PROJECT_ROOT / 'Results' / 'Phase3_Feature_Extraction' / DATASET / 'CNN_LSTM'
...
# LSTM hyperparameters (fixed from Phase 1)
LSTM_FIXED_HPS = None  # Will be loaded from Phase 1 best config

# Import settings from config
N_TRIALS = OPTUNA_N_TRIALS_PHASE3
BATCH_SIZE = CNN_LSTM_FIXED_PARAMS['batch_size']
MAX_EPOCHS = CNN_LSTM_FIXED_PARAMS['max_epochs']
PATIENCE = CNN_LSTM_FIXED_PARAMS['early_stopping_patience']

# ==========
# LOAD PHASE 1 LSTM HYPERPARAMETERS
# ==========

def load_phase1_lstm_hps():
    print("\nLoading Phase 1 LSTM hyperparameters...")
    
    # Try the correct path first
    lstm_hp_file = PROJECT_ROOT / 'Results' / 'Hyperparameter_Tuning' / 'LSTM' / 'LSTM_best_hyperparameters.json'
    
    if lstm_hp_file.exists():
        with open(lstm_hp_file, 'r') as f:
            raw_hps = json.load(f)
        
        # Convert from Optuna format to our format
        lstm_hps = {
            'units': raw_hps.get('units_layer1', 128),
            'num_layers': raw_hps.get('n_lstm_layers', 1),
            'dropout': raw_hps.get('dropout_rate', 0.1),
            'learning_rate': raw_hps.get('learning_rate', 0.001),
            'batch_size': raw_hps.get('batch_size', 32)
        }
        print(f"Loaded LSTM HPs from Hyperparameter_Tuning: {lstm_hps}")
        return lstm_hps
    else:
        print("Phase 1 LSTM HPs not found, using defaults")
        return {
            'units': 128,
            'num_layers': 1,
            'dropout': 0.1,
...
def build_cnn_lstm_model(input_shape, cnn_config, lstm_config):
    inputs = keras.Input(shape=input_shape, name='input')
    x = inputs
    
    # CNN layers for feature extraction
    for i in range(cnn_config['conv_layers']):
        x = layers.Conv1D(
    ...
    for i in range(lstm_config['num_layers']):
        ...
        if lstm_config['dropout'] > 0:
            x = layers.Dropout(lstm_config['dropout'], name=f'dropout_{i+1}')(x)
...
def optuna_objective(trial, train_data, val_data, lstm_config, selected_features):
    # Suggest CNN hyperparameters from config.py search space
    cnn_config = {}
    for param_name, (param_type, param_values) in CNN_LSTM_SEARCH_SPACE.items():
        if param_type == 'categorical':
            cnn_config[param_name] = trial.suggest_categorical(param_name, param_values)
        elif param_type == 'int':
            cnn_config[param_name] = trial.suggest_int(param_name, param_values[0], param_values[1])
        elif param_type == 'float':
            cnn_config[param_name] = trial.suggest_float(param_name, param_values[0], param_values[1])
        elif param_type == 'float_log':
            cnn_config[param_name] = trial.suggest_float(param_name, param_values[0], param_values[1], log=True)
    
    input_shape = (WINDOW_SIZE, len(selected_features))
    
    try:
        # Build model
        model = build_cnn_lstm_model(input_shape, cnn_config, lstm_config)
...
def optimize_cnn_hps(train_data, val_data, lstm_config, selected_features):
    print("\n" + "="*40)
    ...
    study = optuna.create_study(
        ...
        study_name='CNN_LSTM_HP_Optimization'
    )
    
    # Run optimization
    study.optimize(
        lambda trial: optuna_objective(trial, train_data, val_data, lstm_config, selected_features),
        n_trials=N_TRIALS,
    ...
    # Save best config
    best_config_file = OUTPUT_DIR / 'best_cnn_config.json'
...
def evaluate_all_percentages(best_cnn_config, lstm_config, selected_features, engine_counts):
    print("\n" + "="*40)
    print("EVALUATING BEST CONFIG ACROSS ALL DATA PERCENTAGES")
    print("="*40)
    ...
    for n_engines in engine_counts:
        ...
        try:
            ...
            # Build model
            model = build_cnn_lstm_model(input_shape, best_cnn_config, lstm_config)
            ...
            checkpoint_file = OUTPUT_DIR / 'cnn_lstm_all_results_checkpoint.csv'
            ...
            model.save(model_dir / f'cnn_lstm_{n_engines}engines.keras')
    ...
    results_file = OUTPUT_DIR / 'cnn_lstm_all_results.csv'
...
def main(test_mode=False):
    print("\n" + "="*40)
    print("CNN-LSTM FEATURE EXTRACTION EXPERIMENT")
    if test_mode:
    ...
    # Load configurations
    global LSTM_FIXED_HPS
    LSTM_FIXED_HPS = load_phase1_lstm_hps()
    ...
    print(f"\nFixed LSTM config: {LSTM_FIXED_HPS}")
    print(f"Selected features: {len(selected_features)} features")
    
    # Step 1: Hyperparameter optimization on 100% data
    print("\n" + "="*40)
    print("STEP 1: HYPERPARAMETER OPTIMIZATION (100% DATA)")
    print("="*40)
    
    train_data_80, val_data_80, _ = load_windowed_data(80, selected_features)
    best_cnn_config, study = optimize_cnn_hps(
        train_data_80, val_data_80, LSTM_FIXED_HPS, selected_features
    )
    
    # Step 2: Evaluate best config on all engine counts
    print("\n" + "="*40)
    print("STEP 2: EVALUATE ON ALL DATA PERCENTAGES")
    print("="*40)
    
    final_results = evaluate_all_percentages(best_cnn_config, LSTM_FIXED_HPS, selected_features, engine_counts_to_use)
    
    # Save experiment metadata (for thesis documentation)
    metadata = {
        'experiment_name': 'CNN_LSTM_Feature_Extraction',
        'dataset': DATASET,
        ...
        'lstm_fixed_config': LSTM_FIXED_HPS,
    ...
    # Final summary
    print("\n" + "="*40)
    print("CNN-LSTM EXPERIMENT COMPLETE!")
    print("="*40)
    ...
    print(f"best_cnn_config.json - Best CNN configuration found")
    print(f"cnn_lstm_all_results.csv - Full results across all data %")
    print(f"metrics_comparison.csv - Clean metrics table for plotting")
    print(f"summary_statistics.csv - Aggregated statistics")
    print(f"training_history/ - Per-epoch loss curves for each data %")
    print(f"models/ - Trained .keras models for each data %")
    print()
