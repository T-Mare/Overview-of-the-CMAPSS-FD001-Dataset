...
def compute_label_layout(line_endpoints, y_min, y_max, min_gap):
    if not line_endpoints:
        return []

    # Sort from highest visible endpoint (worst) to lowest (best).
    sorted_endpoints = sorted(
        line_endpoints,
        key=lambda x: (float(np.clip(x[0], y_min, y_max)), x[0]),
        reverse=True
    )

    positioned_endpoints = []
    next_y = y_max

    for rmse_val, model, color in sorted_endpoints:
        visible_y = float(np.clip(rmse_val, y_min, y_max))
        label_y = min(visible_y, next_y)
        positioned_endpoints.append((rmse_val, visible_y, label_y, model, color))
        next_y = label_y - min_gap

    # If labels still do not fit cleanly, fall back to evenly spaced positions
    # while preserving the ranking order.
    if positioned_endpoints and positioned_endpoints[-1][2] < y_min:
        fallback_positions = np.linspace(y_max, y_min, len(positioned_endpoints))
        positioned_endpoints = [
            (rmse_val, visible_y, float(label_y), model, color)
            for (rmse_val, visible_y, _, model, color), label_y
            in zip(positioned_endpoints, fallback_positions)
        ]

    return positioned_endpoints

def plot_rmse_evolution(df, model_family, output_dir, engine_range, range_label):
    print(f"\nPlotting RMSE evolution for {model_family} models ({range_label})...")
    ...
    line_endpoints = []  # Store (rmse_at_10pct, model_name, color) for labeling
    ...
    for model in sorted(models):
        ...
        plt.plot(model_data['n_engines'], model_data['rmse'],
                linewidth=2.2,
                label=model, color=colors[model], alpha=0.9)
        
        # Get RMSE at lowest engine count in this range for labeling
        min_engines = min(engine_range)
        rmse_at_min = model_data[model_data['n_engines'] == min_engines]['rmse'].values
        if len(rmse_at_min) > 0:
            line_endpoints.append((rmse_at_min[0], model, colors[model]))
    ...
    if range_label == "high_engines":
        ...
        line_start_x = min_engines  # Start connector from lowest engine count (1)

    positioned_endpoints = compute_label_layout(line_endpoints, y_min, y_max, min_gap)

    for _, visible_y, label_y, model, color in positioned_endpoints:
        
        # Draw connector line from actual data point to label
        plt.plot([line_start_x, label_x], [visible_y, label_y], 
    ...
    plt.ylabel('RMSE (Validation Set)', fontsize=13, fontweight='bold')
    
    range_title = "High Engine Counts (80-10)" if range_label == "high_engines" else "Low Engine Counts (10-1)"
    plt.title(f'{model_family} Models: RMSE Evolution - {range_title}',
    ...
    if range_label == "high_engines":
        ...
        plt.ylim(10, 80)  # More range for low data (worse performance)
        plt.xlim(max(engine_range) + 1, min_engines - 3)  # Extra space on left for labels
    
    plt.tight_layout()
    
    # Save
    filename = f"{model_family.lower().replace('-', '_').replace(' ', '_')}_rmse_evolution_{range_label}.png"
    output_file = output_dir / filename
    plt.savefig(output_file, dpi=300)
    print(f"Saved: {output_file}")
    plt.close()

def plot_combined_baseline_evaluation(regression_df, tree_df, dl_df, output_dir, engine_range, range_label):
    print(f"\nPlotting combined baseline evaluation (All 3 families) ({range_label})...")
    ...
    plt.ylabel('RMSE (Validation Set)', fontsize=14, fontweight='bold')
    
    range_title = "High Engine Counts (80-10)" if range_label == "high_engines" else "Low Engine Counts (10-1)"
    plt.title(f'All Baseline Models: RMSE Evolution - {range_title}\n(Regression, Tree-Based, and Deep Learning)',
    ...
    # Save
    output_file = output_dir / f'combined_baseline_evaluation_{range_label}.png'
    plt.savefig(output_file, dpi=300)
    print(f"Saved: {output_file}")
    plt.close()
...
def main():
    print("="*40)
    print("ALL BASELINE MODELS EVALUATION")
    print("="*40)
    ...
    # Plot RMSE evolution for Regression models (high engines)
    plot_rmse_evolution(regression_df, 'Regression', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot RMSE evolution for Tree-Based models (high engines)
    plot_rmse_evolution(tree_df, 'Tree-Based', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot RMSE evolution for Deep Learning models (high engines)
    plot_rmse_evolution(dl_df, 'Deep Learning', OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    # Plot combined evaluation (high engines)
    plot_combined_baseline_evaluation(regression_df, tree_df, dl_df, OUTPUT_DIR, ENGINE_COUNTS_HIGH, 'high_engines')
    
    print("\n" + "="*40)
    print("GENERATING LOW ENGINE COUNT PLOTS (10-1 engines)")
    print("="*40)
    
    # Plot RMSE evolution for Regression models (low engines)
    plot_rmse_evolution(regression_df, 'Regression', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot RMSE evolution for Tree-Based models (low engines)
    plot_rmse_evolution(tree_df, 'Tree-Based', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot RMSE evolution for Deep Learning models (low engines)
    plot_rmse_evolution(dl_df, 'Deep Learning', OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    
    # Plot combined evaluation (low engines)
    plot_combined_baseline_evaluation(regression_df, tree_df, dl_df, OUTPUT_DIR, ENGINE_COUNTS_LOW, 'low_engines')
    ...
    print("\nGenerated files:")
    print("1. regression_rmse_evolution.png")
    print("2. tree_based_rmse_evolution.png")
    print("3. deep_learning_rmse_evolution.png")
    print("4. combined_baseline_evaluation.png (ALL MODELS)")
    print("="*40)
