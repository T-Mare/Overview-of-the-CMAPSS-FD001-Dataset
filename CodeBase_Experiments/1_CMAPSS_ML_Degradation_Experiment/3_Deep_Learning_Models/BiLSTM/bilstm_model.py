...
MODEL_INFO = {
    ...
    'description': 'Bidirectional LSTM network with 1-2 layers (processes sequences forward & backward)'
}
...
def build_model(n_recurrent_layers, units_layer1, units_layer2,
                ...
                merge_mode='concat', input_shape=(30, 24), random_seed=42):
    ...
    if n_recurrent_layers == 1:
        ...
        # Second Bidirectional LSTM layer (final layer: return sequences = False)
        model.add(layers.Bidirectional(
            layers.LSTM(
                units_layer2,
                dropout=dropout_rate,
...
def save_model(model, history, params, save_dir, data_percentage):
    ...
    model_path = save_dir / f'BiLSTM_{data_percentage}pct.keras'
    model.save(model_path)
    
    # Save hyperparameters
    params_path = save_dir / f'BiLSTM_{data_percentage}pct_params.json'
    with open(params_path, 'w') as f:
        json.dump(params, f, indent=4)
    
    # Save training history
    history_path = save_dir / f'BiLSTM_{data_percentage}pct_history.json'
    history_dict = {
...
if __name__ == "__main__":
    """
    Test BiLSTM model building and training with dummy data
    """
    print("="*40)
    ...
    model = build_model(
        ...
        merge_mode=test_params['merge_mode']
    )
