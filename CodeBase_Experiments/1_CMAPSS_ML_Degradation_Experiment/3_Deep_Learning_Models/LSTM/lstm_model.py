...
MODEL_INFO = {
    ...
    'description': 'LSTM network with 1-2 LSTM layers for temporal dependencies'
}
...
def build_model(n_recurrent_layers, units_layer1, units_layer2,
    ...
    if n_recurrent_layers == 1:
        ...
        # Second LSTM layer (final layer: return sequences = False)
        model.add(layers.LSTM(
            units_layer2,
            dropout=dropout_rate,
            recurrent_dropout=recurrent_dropout,
...
def save_model(model, history, params, save_dir, data_percentage):
    ...
    model_path = save_dir / f'LSTM_{data_percentage}pct.keras'
    model.save(model_path)
    
    # Save hyperparameters
    params_path = save_dir / f'LSTM_{data_percentage}pct_params.json'
    with open(params_path, 'w') as f:
        json.dump(params, f, indent=4)
    
    # Save training history
    history_path = save_dir / f'LSTM_{data_percentage}pct_history.json'
    history_dict = {
...
if __name__ == "__main__":
    """
    Test LSTM model building and training with dummy data
    """
    print("="*40)
