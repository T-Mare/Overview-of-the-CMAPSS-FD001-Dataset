...
MODEL_INFO = {
    ...
    'description': 'Simple recurrent network with 1-2 RNN layers'
}
...
def build_model(n_recurrent_layers, units_layer1, units_layer2,
    ...
    if n_recurrent_layers == 1:
        ...
        # Second RNN layer (final layer: return sequences = False)
        model.add(layers.SimpleRNN(
            units_layer2,
            activation='tanh',
            dropout=dropout_rate,
...
def save_model(model, history, params, save_dir, data_percentage):
    ...
    model_path = save_dir / f'RNN_{data_percentage}pct.keras'
    model.save(model_path)
    
    # Save hyperparameters
    params_path = save_dir / f'RNN_{data_percentage}pct_params.json'
    with open(params_path, 'w') as f:
        json.dump(params, f, indent=4)
    
    # Save training history
    history_path = save_dir / f'RNN_{data_percentage}pct_history.json'
    history_dict = {
...
if __name__ == "__main__":
    """
    Test RNN model building and training with dummy data
    """
    print("="*40)
