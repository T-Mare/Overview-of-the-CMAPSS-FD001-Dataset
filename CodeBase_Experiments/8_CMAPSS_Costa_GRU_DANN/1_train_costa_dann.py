...
# Costa et al. hyperparameters (GRU variant) - Source-Target specific configurations
...
def train_costa_dann(n_engines, hyperparams, baseline_df=None):
    ...
    print("\nBuilding GRU-DANN model...")
    model = build_costa_dann_fd002_to_fd001(
        input_shape=(30, 14),
        gru_units=hyperparams['gru_units'],
        gru_dropout=hyperparams['gru_dropout'],
    ...
    print(
        f"Architecture: GRU({hyperparams['gru_units']}), "
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Costa et al. GRU-DANN (GRU + GRL) with project preprocessed data."
    )
    parser.add_argument(
...
def main():
    ...
    print(f"Engine counts: {engine_counts}")
    print(f"\nConfiguration (GRU variant):")
    print(f"GRU: {hyperparams['gru_units']} units, dropout={hyperparams['gru_dropout']}")
    print(f"Feature: {hyperparams['feature_units']} units")
    print(f"RUL: {hyperparams['rul_units']} units, dropout={hyperparams['rul_dropout']}")
    print(f"Domain: {hyperparams['domain_units']}, dropout={hyperparams['domain_dropout']}")
