...
# Phase 2 Baseline path (LSTM + Correlation_FS, no transfer learning)
# __file__ = .../3_CMAPSS_Intra_TL_Degradation_Experiment/2_finetune_vanilla_tl.py
...
def load_phase2_baseline():
    ...
    # Filter for LSTM + Correlation_FS
    baseline_df = baseline_df[baseline_df['fs_method'] == 'Correlation_FS']
    
    print(f"Loaded Phase 2 baseline (LSTM + Correlation_FS)")
    return baseline_df
...
def finetune_model(model, X_train, y_train, X_val, y_val):
    print("\nFine-tuning model on target data...")
...
def evaluate_model(model, X_val, y_val, X_test, y_test, n_engines, baseline_df=None):
    ...
    if baseline_df is not None:
        ...
        if not baseline_row.empty:
            ...
            print(f"\n   Transfer Learning Gain vs Phase 2 Baseline (LSTM + Correlation_FS, no TL):")
...
def save_finetuned_model(model, history, metrics, finetune_time, 
    ...
    save_dir = OUTPUT_BASE / f'{source_dataset}_to_{TARGET_DATASET}' / 'TL1_Vanilla_FT'
    save_dir.mkdir(parents=True, exist_ok=True)
    
    # Save model
    model_path = save_dir / f'tl1_model_{n_engines}engines.keras'
    model.save(model_path)
    
    # Save history (for convergence plots)
    history_path = save_dir / f'tl1_history_{n_engines}engines.json'
    history_dict = {
    ...
    # Save metrics
    metrics_path = save_dir / f'tl1_metrics_{n_engines}engines.json'
    ...
    metrics['tl_strategy'] = 'TL1_Vanilla_FT'
    ...
    if y_val_pred is not None and y_val_true is not None:
        ...
        val_pred_df.to_csv(pred_dir / f'tl1_val_predictions_{n_engines}engines.csv', index=False)
    ...
    if y_test_pred is not None and y_test_true is not None:
        ...
        test_pred_df.to_csv(pred_dir / f'tl1_test_predictions_{n_engines}engines.csv', index=False)
...
def run_vanilla_tl(source_dataset, n_engines, baseline_df=None):
    print("\n" + "="*40)
    print(f"TL1: VANILLA FINE-TUNING")
...
def run_all_experiments():
    ...
    print("\n" + "="*40)
    print("PHASE 4: TL1 - VANILLA FINE-TUNING (ALL EXPERIMENTS)")
    print("="*40)
    ...
    for source_dataset in SOURCE_DATASETS:
        for n_engines in ENGINE_COUNTS_ALL:
            try:
                metrics = run_vanilla_tl(source_dataset, n_engines, baseline_df)
    ...
    if all_results:
        ...
        for source in results_df['source'].unique():
            ...
            results_path = OUTPUT_BASE / f'TL1_{source}_all_results.csv'
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='TL1: Vanilla Fine-Tuning')
    ...
    if args.source == 'all' or args.n_engines == 0:
        ...
        baseline_df = load_phase2_baseline()
        run_vanilla_tl(args.source, args.n_engines, baseline_df)
