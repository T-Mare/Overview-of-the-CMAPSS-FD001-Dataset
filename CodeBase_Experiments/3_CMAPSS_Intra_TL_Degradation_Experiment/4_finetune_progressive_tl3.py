...
TL3_STAGE_PARAMS = {
    'stage1': {  # Frozen features (like TL2)
        ...
        'learning_rate': 0.001,
        'patience': 5,
        'unfreeze_layers': []  # All LSTM layers frozen
    },
    'stage2': {  # Unfreeze last LSTM layer
        'name': 'Last LSTM + Output',
        'epochs': 10,
        'learning_rate': 0.0005,  # Lower than stage 1
        'patience': 5,
        'unfreeze_layers': ['lstm_1']  # Unfreeze last LSTM
    },
    'stage3': {  # Unfreeze all
        'name': 'All Layers',
        'epochs': 30,
        ...
        'patience': 5,
        'unfreeze_layers': ['lstm', 'lstm_1']  # All LSTM layers
    }
}
...
def progressive_finetune_tl3(model, X_train, y_train, X_val, y_val):
    print("\n" + "="*40)
    print("TL3: PROGRESSIVE UNFREEZING FINE-TUNING")
    print("="*40)
    print("\nStrategy: 3-stage gradual unfreezing")
    print("Stage 1: Output only (frozen features)")
    print("Stage 2: Last LSTM + Output")
    ...
    print("\n" + "="*40)
    print("TL3 PROGRESSIVE UNFREEZING COMPLETE")
    print("="*40)
...
def evaluate_tl3_model(model, X_val, y_val, X_test, y_test, n_engines, baseline_df=None):
    print("\nEvaluating TL3 fine-tuned model...")
    ...
    if baseline_df is not None:
        ...
        if not baseline_row.empty:
            ...
            print(f"\n   TL3 Gain vs Phase 2 Baseline (LSTM + Correlation_FS, no TL):")
...
def save_tl3_results(model, all_histories, metrics, total_time, stage_times,
    ...
    save_dir = OUTPUT_BASE / f'{source_dataset}_to_{TARGET_DATASET}' / 'TL3_Progressive_Unfreeze'
    save_dir.mkdir(parents=True, exist_ok=True)
    
    # Save model
    model_path = save_dir / f'tl3_model_{n_engines}engines.keras'
    model.save(model_path)
    
    # Save combined history (all 3 stages)
    history_path = save_dir / f'tl3_history_{n_engines}engines.json'
    combined_history = {}
    ...
    # Save metrics
    metrics_path = save_dir / f'tl3_metrics_{n_engines}engines.json'
    ...
    metrics['tl_strategy'] = 'TL3_Progressive_Unfreeze'
    ...
    if y_val_pred is not None and y_val_true is not None:
        ...
        val_pred_df.to_csv(pred_dir / f'tl3_val_predictions_{n_engines}engines.csv', index=False)
    ...
    if y_test_pred is not None and y_test_true is not None:
        ...
        test_pred_df.to_csv(pred_dir / f'tl3_test_predictions_{n_engines}engines.csv', index=False)
...
def run_tl3(source_dataset, n_engines, baseline_df=None):
    print("\n" + "="*40)
    print(f"TL3: PROGRESSIVE UNFREEZING")
    ...
    all_histories, total_time, stage_times = progressive_finetune_tl3(
...
def run_all_tl3_experiments():
    print("\n" + "="*40)
    print("PHASE 4: TL3 - PROGRESSIVE UNFREEZING (ALL EXPERIMENTS)")
    print("="*40)
    ...
    for source_dataset in SOURCE_DATASETS:
        for n_engines in ENGINE_COUNTS_ALL:
            try:
                metrics = run_tl3(source_dataset, n_engines, baseline_df)
    ...
    if all_results:
        ...
        for source in results_df['source'].unique():
            ...
            results_path = OUTPUT_BASE / f'TL3_{source}_all_results.csv'
            results_path.parent.mkdir(parents=True, exist_ok=True)
            source_results.to_csv(results_path, index=False)
            print(f"\n {source} TL3 results saved: {results_path}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='TL3: Progressive Unfreezing Fine-Tuning')
    ...
    if args.source == 'all' or args.n_engines == 0:
        ...
        baseline_df = load_phase2_baseline()
        run_tl3(args.source, args.n_engines, baseline_df)
