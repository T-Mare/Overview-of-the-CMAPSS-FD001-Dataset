...
def load_and_freeze_model(source_dataset):
    ...
    # FREEZE ALL GRU LAYERS (TL2 key step!)
    print(f"\n Freezing GRU layers (TL2 strategy)...")
    frozen_count = 0
    trainable_count = 0
...
def finetune_tl2_model(model, X_train, y_train, X_val, y_val):
    print("\nFine-tuning TL2-GRU model (frozen GRU features)...")
    ...
    print(f"\n TL2-GRU fine-tuning complete in {finetune_time:.1f}s ({finetune_time/60:.1f} min)")
...
def evaluate_tl2_model(model, X_val, y_val, X_test, y_test, n_engines, baseline_df=None):
    print("\nEvaluating TL2-GRU fine-tuned model...")
    ...
    if baseline_df is not None:
        ...
        if not baseline_row.empty:
            ...
            print(f"\n   TL2-GRU Gain vs Phase 2 Baseline (GRU + Correlation_FS, no TL):")
...
def save_tl2_model(model, history, metrics, finetune_time, 
    ...
    save_dir = OUTPUT_BASE / f'{source_dataset}_to_{TARGET_DATASET}' / 'TL2_Frozen_FT_GRU'
    save_dir.mkdir(parents=True, exist_ok=True)
    
    # Save model
    model_path = save_dir / f'tl2_gru_model_{n_engines}engines.keras'
    model.save(model_path)
    
    # Save history
    history_path = save_dir / f'tl2_gru_history_{n_engines}engines.json'
    history_dict = {
    ...
    # Save metrics
    metrics_path = save_dir / f'tl2_gru_metrics_{n_engines}engines.json'
    ...
    metrics['tl_strategy'] = 'TL2_Frozen_FT_GRU'
    ...
    if y_val_pred is not None and y_val_true is not None:
        ...
        val_pred_df.to_csv(pred_dir / f'tl2_gru_val_predictions_{n_engines}engines.csv', index=False)
    ...
    if y_test_pred is not None and y_test_true is not None:
        ...
        test_pred_df.to_csv(pred_dir / f'tl2_gru_test_predictions_{n_engines}engines.csv', index=False)
...
def run_tl2_gru(source_dataset, n_engines, baseline_df=None):
    print("\n" + "="*40)
    print(f"TL2-GRU: FROZEN FEATURES FINE-TUNING")
    print(f"Source: {source_dataset}  Target: {TARGET_DATASET} ({n_engines} engines)")
    print(f"Strategy: Freeze GRU layers, train only Dense output layer")
    print("="*40)
    ...
    print(f"\n TL2-GRU results saved: {save_dir}")
    print("="*40)
...
def run_all_tl2_experiments():
    ...
    for source_dataset in SOURCE_DATASETS:
        for n_engines in ENGINE_COUNTS_ALL:
            try:
                metrics = run_tl2_gru(source_dataset, n_engines, baseline_df)
    ...
    if all_results:
        ...
        for source in results_df['source'].unique():
            ...
            results_path = OUTPUT_BASE / f'TL2_GRU_{source}_all_results.csv'
            results_path.parent.mkdir(parents=True, exist_ok=True)
            source_results.to_csv(results_path, index=False)
            print(f"\n {source} TL2-GRU results saved: {results_path}")
        
        # Summary
        print("\n" + "="*40)
        print("TL2-GRU SUMMARY")
        print("="*40)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='TL2-GRU: Frozen Features Fine-Tuning with GRU')
    ...
    if args.source == 'all' or args.n_engines == 0:
        ...
        baseline_df = load_phase2_baseline()
        run_tl2_gru(args.source, args.n_engines, baseline_df)
