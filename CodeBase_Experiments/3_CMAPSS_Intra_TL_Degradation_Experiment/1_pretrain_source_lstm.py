...
# Phase 1 LSTM best hyperparameters
LSTM_HYPERPARAMS = {
    'n_lstm_layers': 1,
    'units_layer1': 128,
    'units_layer2': 64,  # Not used (only 1 layer)
    'dropout_rate': 0.1,
...
def build_lstm_model(input_shape=(30, 14)):
    # Set random seeds
    np.random.seed(RANDOM_SEED)
    tf.random.set_seed(RANDOM_SEED)
    
    model = keras.Sequential(name='LSTM_Pretrain')
    
    # Input layer
    model.add(keras.layers.Input(shape=input_shape, name='input'))
    
    # LSTM layer (1 layer, 128 units)
    model.add(keras.layers.LSTM(
        units=LSTM_HYPERPARAMS['units_layer1'],
        dropout=LSTM_HYPERPARAMS['dropout_rate'],
        recurrent_dropout=LSTM_HYPERPARAMS['recurrent_dropout'],
    ...
    # Compile
    optimizer = keras.optimizers.Adam(learning_rate=LSTM_HYPERPARAMS['learning_rate'])
    model.compile(
        optimizer=optimizer,
        loss='mse',
        metrics=['mae']
...
def save_pretrained_model(model, history, metrics, training_time, source_dataset):
    ...
    # Save model
    model_path = save_dir / f'lstm_pretrained_{source_dataset}.keras'
    model.save(model_path)
    print(f"\n Model saved: {model_path}")
    
    # Save hyperparameters
    params_path = save_dir / f'hyperparameters_{source_dataset}.json'
    with open(params_path, 'w') as f:
        json.dump(LSTM_HYPERPARAMS, f, indent=2)
    
    # Save training history
    history_path = save_dir / f'training_history_{source_dataset}.json'
    history_dict = {
    ...
    metrics_path = save_dir / f'pretrain_metrics_{source_dataset}.json'
...
def pretrain_source_model(source_dataset):
    print("\n" + "="*40)
    print(f"PRE-TRAINING LSTM ON {source_dataset}")
    print("="*40)
...
if __name__ == '__main__':
    print("\n" + "="*40)
    print("PHASE 4: TRANSFER LEARNING - PRE-TRAINING SCRIPT")
    print("="*40)
    print(f"Source datasets: {SOURCE_DATASETS}")
    print(f"Features: 14 Correlation_FS features")
    print(f"Architecture: {LSTM_HYPERPARAMS['n_lstm_layers']} LSTM layer, {LSTM_HYPERPARAMS['units_layer1']} units")
